...
#include <ctime>// for using time functions to log transactions with timestamps
#include <fstream>// for file input/output operations to save and load account data and transaction in files
#include <iomanip>// for تنسيق الوقت by using put_time to format the timestamp in the transaction log
#include <iostream>
#include <sstream>// for dividing the line read from the file into separate components (type, accountId, ownerName, balance)
#include <stdexcept> // for using standard exception classes like runtime_error and invalid_argument to handle errors
